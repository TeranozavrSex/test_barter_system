...
from .serializers import (
    ...
    ExchangeProposalListSerializer,
...
@aboba_swagger(
    ...
    query_params={"ad_id": int},
    body_params={"ad_sender": int, "comment": str},
    responses={
...
def create_proposal(request, ad_id):
    ...
    if request.method == "POST":
        ...
        ).exclude(sent_proposals__ad_receiver=ad_receiver)

    return render(
        request,
        "barter/create_proposal.html",
...
@aboba_swagger(
    ...
    query_params={"proposal_id": int, "status": str},
    responses={
...
def update_proposal_status(request, proposal_id, status):
    ...
    if status == "accepted":
        ...
        ExchangeProposal.objects.filter(
            ad_sender=proposal.ad_sender, status="pending"
        ).exclude(id=proposal.id).update(status="cancelled")

        ExchangeProposal.objects.filter(
            ad_receiver=proposal.ad_sender, status="pending"
        ).exclude(id=proposal.id).update(status="cancelled")

        ExchangeProposal.objects.filter(
            ad_sender=proposal.ad_receiver, status="pending"
        ).exclude(id=proposal.id).update(status="cancelled")

        ExchangeProposal.objects.filter(
            ad_receiver=proposal.ad_receiver, status="pending"
        ).exclude(id=proposal.id).update(status="cancelled")
...
@aboba_swagger(
    ...
    responses={
        ...
        "404": {"detail": "Объявление не найдено"},
    },
    tags=["api"],
    is_drf=False,
)
...
@aboba_swagger(
    ...
    responses={
        ...
        "401": {"detail": "Учетные данные не были предоставлены."},
        "403": {"detail": "У вас нет прав редактировать это объявление."},
        "404": {"detail": "Объявление не найдено."},
    },
    need_auth=True,
    tags=["api"],
...
@aboba_swagger(
    ...
    query_params={"pk": int},
    responses={
        "204": {"detail": "Объявление удалено."},
        "401": {"detail": "Учетные данные не были предоставлены."},
        "403": {"detail": "У вас нет прав удалить это объявление."},
        "404": {"detail": "Объявление не найдено."},
    },
    need_auth=True,
    tags=["api"],
...
@aboba_swagger(
    ...
    responses={
        ...
        "401": {"detail": "Учетные данные не были предоставлены."},
        "403": {"detail": "У вас нет доступа к этому предложению обмена."},
        "404": {"detail": "Предложение обмена не найдено."},
    },
    need_auth=True,
    tags=["api"],
)
...
def proposal_create_api(request, ad_id):
    try:
        ...
        serializer = ExchangeProposalCreateSerializer(
            data=request.data, context={"request": request, "ad_receiver": ad_receiver}
        )
...
@aboba_swagger(
    ...
    responses={
        ...
        "401": {"detail": "Учетные данные не были предоставлены."},
        "403": {"detail": "У вас нет прав для изменения этого предложения."},
        "404": {"detail": "Предложение обмена не найдено."},
    },
    need_auth=True,
    tags=["api"],
...
def proposal_update_api(request, pk):
    try:
        ...
        serializer = ExchangeProposalUpdateSerializer(
