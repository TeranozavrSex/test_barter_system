...
class ExchangeProposalModelTest(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(
            username="sender", password="testpass123", email="sender@example.com"
        )

        self.user2 = User.objects.create_user(
            username="receiver", password="testpass123", email="receiver@example.com"
        )
...
class AdViewWithAuthTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user.token_hash = create_token(
            "127.0.0.1", f"timestamp{timezone.now()}user_id{self.user.id}"
        )
    ...
    def test_ad_update_view(self):
        ...
        response = self.client.post(
            reverse("barter:ad_update", args=[self.ad.pk]), data
        )
...
class ExchangeProposalViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user1.token_hash = create_token(
            "127.0.0.1", f"timestamp{timezone.now()}user_id{self.user1.id}"
        )
        self.user1.token_created_at = timezone.now()
        self.user1.save()

        # Create token for user2
        self.user2.token_hash = create_token(
            "127.0.0.1", f"timestamp{timezone.now()}user_id{self.user2.id}"
        )
    ...
    def test_my_proposals_view(self):
        ...
        self.assertContains(
            response, "Receiver Ad"
        )  # Should contain receiver's ad title
    ...
    def test_create_proposal_view(self):
        ...
        response = self.client.get(
            reverse("barter:create_proposal", args=[self.ad1.id])
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertTrue(
            ExchangeProposal.objects.filter(
                ad_sender=ad3, ad_receiver=self.ad1
            ).exists()
        )
    ...
    def test_update_proposal_status_accepted(self):
        ...
        response = self.client.get(
            reverse(
                "barter:update_proposal_status", args=[self.proposal.id, "accepted"]
            )
        )
        self.assertEqual(response.status_code, 302)  # Should redirect after update
    ...
    def test_update_proposal_status_rejected(self):
        ...
        response = self.client.get(
            reverse(
                "barter:update_proposal_status", args=[self.proposal.id, "rejected"]
            )
        )
        self.assertEqual(response.status_code, 302)  # Should redirect after update
...
class APITestBase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="apiuser", password="apipass123", email="api@example.com"
        )

        # Create token using the same method as in the real application
        self.user_token = create_token(
            "127.0.0.1", f"timestamp{timezone.now()}user_id{self.user.id}"
        )
        ...
        self.user2 = User.objects.create_user(
            username="apiuser2", password="apipass123", email="api2@example.com"
        )

        self.user2_token = create_token(
            "127.0.0.1", f"timestamp{timezone.now()}user_id{self.user2.id}"
        )
...
class APIAdTests(APITestBase):
    ...
    def test_ad_create_api(self):
        ...
        response = self.client.post(
            reverse("barter:api_ad_create"), data, format="json"
        )
    ...
    def test_ad_update_api(self):
        ...
        response = self.client.patch(
            reverse("barter:api_ad_update", args=[self.ad.id]), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_ad_delete_api(self):
        ...
        response = self.client.delete(
            reverse("barter:api_ad_delete", args=[self.ad.id])
        )
...
class APIProposalTests(APITestBase):
    ...
    def test_proposal_detail_api(self):
        ...
        response = self.client.get(
            reverse("barter:api_proposal_detail", args=[self.proposal.id])
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_create_proposal_api(self):
        ...
        data = {"ad_sender": ad3.id, "comment": "Another API exchange proposal"}
        response = self.client.post(
            reverse("barter:api_proposal_create", args=[self.ad.id]),
            data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_update_proposal_status_api(self):
        ...
        response = self.client.patch(
            reverse("barter:api_proposal_update", args=[self.proposal.id]),
            data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
