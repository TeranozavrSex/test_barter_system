...
class ExchangeProposalCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_ad_sender(self, value):
        ...
        if value.user != user:
            raise serializers.ValidationError(
                "Вы можете предлагать только свои объявления"
            )
    ...
    def validate(self, data):
        ...
        if "ad_sender" in data and ad_receiver:
            if data["ad_sender"].user == ad_receiver.user:
                raise serializers.ValidationError(
                    {"ad_sender": "Нельзя предлагать обмен на свое же объявление"}
                )

            # Проверка уникальности
            existing = ExchangeProposal.objects.filter(
                ad_sender=data["ad_sender"], ad_receiver=ad_receiver, status="pending"
            ).exists()

            if existing:
                raise serializers.ValidationError(
                    {
                        "ad_sender": "Предложение обмена между этими объявлениями уже существует"
                    }
                )
